import { DbConnection, DbConnectionBuilder, reducers, tables } from "./module_bindings";
import type {
  Agent,
  Message,
  MessagePart,
...
export function extractTimestamp(ts: unknown): number {
  ...
  if (ts && typeof ts === "object" && "__timestamp_micros_since_unix_epoch__" in ts) {
    return Number(
      (ts as { __timestamp_micros_since_unix_epoch__: bigint })
        .__timestamp_micros_since_unix_epoch__ / 1000n
    );
  }
